...
class Pagination {
	...
	public function render() {
		...
		if ($page > 1) {
			...
			if ($page - 1 === 1) {
				$output .= '<a href="' . str_replace(array('&amp;page={page}', '?page={page}', '&page={page}'), '', $this->url) . '" class="prev"></a>';
			} else {
				$output .= '<a href="' . str_replace('{page}', $page - 1, $this->url) . '" class="prev" > </a>';
			}
		}
		...
		if ($num_pages > 1) {
			...
			for ($i = $start; $i <= $end; $i++) {
				if ($page == $i) {
					$output .= '<a href="javascript:void(0)" class="active" title="page 1">' . $i . '</a>';
				} else {
					if ($i === 1) {
