...
// Product List
	function addtocuart () {
		...
		var quantity = 1;
		$('#addproductid').load('index.php?route=extension/module/sku&sku='+articul, function(response) {
 
		$.ajax({
			url: 'index.php?route=checkout/cart/add',
			type: 'post',
			data: 'product_id=' + response + '&quantity=' + (typeof(quantity) != 'undefined' ? quantity : 1),
			dataType: 'json',
			beforeSend: function() {
